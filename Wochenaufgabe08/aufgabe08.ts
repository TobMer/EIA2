namespace l08aufgabe {  // ich habe mein bestes getan und hab ewig gebraucht. Zwar kam des etwas spät aber über ein Feedback freue ich mich
    ...
    function zeichneVögel(_position: Vector, _size: Vector) {

        let nVögel: number = 21;
        let radiusVogel: number = 7 + Math.random() * 10;
        let vogel: Path2D = new Path2D();

        vogel.arc(0, 0, radiusVogel, 0, 2 * Math.PI);

        let kopf: number = 0 - radiusVogel;
